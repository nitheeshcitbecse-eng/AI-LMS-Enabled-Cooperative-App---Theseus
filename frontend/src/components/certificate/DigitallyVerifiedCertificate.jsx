...
export const DigitallyVerifiedCertificate = ({ certificate: certificateProp, showActions = true }) => {
  ...
  const certificate = certificateProp ?? certificates[0];
  const handlePrint = () => {
    window.print();
  };

  const handleDownloadPDF = () => {
    alert(`Downloading Sample PDF Certificate for ${trainee.name} (${certificate.credentialId})...`);
  };
    ...
    <div className="space-y-4">
      ...
      <div className="p-6 sm:p-10 lg:p-12 rounded-3xl bg-gradient-to-b from-amber-50/60 via-white to-amber-50/40 border-8 border-double border-slate-900/30 shadow-2xl relative text-center space-y-6 select-none overflow-hidden">
        ...
        <div className="space-y-2 pt-2">
          <div className="w-16 h-16 sm:w-20 sm:h-20 rounded-full bg-indigo-800 text-amber-300 font-serif font-black text-xl sm:text-2xl flex items-center justify-center mx-auto shadow-xl border-4 border-amber-400">
            NCCT
          </div>
        ...
        <div className="space-y-4 py-2">
          ...
          <div className="py-2">
            ...
            <p className="text-xs text-slate-600 font-mono mt-2 font-bold">
              Roll No: <span className="text-indigo-950">{trainee.id}</span> • Batch ID: <span className="text-indigo-950">{trainee.batch}</span>
            </p>
          </div>
          ...
          <div className="p-4 sm:p-5 rounded-2xl bg-indigo-800 text-white max-w-2xl mx-auto shadow-lg border-2 border-amber-400/80 space-y-1.5">
            <h3 className="text-base sm:text-lg font-bold text-amber-300 font-sans tracking-wide">
              {certificate.title}
            </h3>
            <p className="text-xs text-slate-300 font-sans">
              Evaluated at <span className="font-bold text-white">{trainee.institute}</span> • Grade: <span className="font-extrabold text-emerald-400">Pass with Distinction (88.4%)</span>
            </p>
          </div>
        </div>
        ...
        <div className="pt-8 border-t-2 border-slate-300/80 grid grid-cols-3 items-end gap-4 text-left">
          ...
          <div className="text-center space-y-1">
            ...
            <p className="text-[9px] sm:text-[10px] text-slate-500">{trainee.institute}</p>
          </div>
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 p-4 bg-white rounded-2xl border border-slate-200 shadow-xs">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-5 py-2 rounded-lg bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors flex items-center gap-2 shadow-sm cursor-pointer"
            >
