...
export const ClosedLoopInterventionModal = () => {
  ...
  const handleSubmitQuiz = () => {
    let correctCount = 0;
    aiRecommendation.quizQuestions.forEach(q => {
      if (selectedAnswers[q.id] === q.correctAnswer) {
        correctCount += 1;
      }
    });

    const finalScore = Math.round((correctCount / aiRecommendation.quizQuestions.length) * 100);
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-lg font-semibold text-slate-900">AI Closed-Loop Learning Intervention</h3>
              <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold">
                15-Min Booster
              </span>
            </div>
            <p className="text-xs text-slate-500">
              Topic: {aiRecommendation.topic} (Original Score: 46%)
            </p>
          </div>
        </div>
        ...
        {!submitted ? (
          <div className="space-y-6">
            ...
            <div className="space-y-5">
              ...
              {aiRecommendation.quizQuestions.map((q, idx) => (
                      ...
                      return (
                        <button
                          ...
                          className={`w-full text-left p-2.5 rounded-lg text-xs font-medium border transition-all flex items-center justify-between ${
                            isSelected
                              ? 'bg-indigo-600 text-white border-indigo-900 shadow-xs'
            ...
            <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-full bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors shadow-sm disabled:opacity-50"
              >
