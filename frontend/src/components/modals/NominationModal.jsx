...
import { X, FileText, CheckCircle2, Building, Download, Sparkles } from 'lucide-react';
import { useApp } from '../../context/AppContext';
import { useData } from '../../context/DataContext';

export const NominationModal = () => {
  const { nominationInfo } = useData();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl ring-1 ring-slate-200 animate-in zoom-in-95 duration-300 space-y-5 relative max-h-[90vh] overflow-y-auto">
        <button
          onClick={closeModal}
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <h3 className="text-lg font-semibold text-slate-900">Programme Registration & Nomination Ledger</h3>
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 space-y-3 text-xs">
          <div className="flex items-center justify-between">
            ...
            <span className="tabular-nums font-bold text-indigo-900">{nominationInfo.nominationRefNo}</span>
          </div>

          <div className="space-y-1 bg-white p-3 rounded-lg border border-slate-200">
            <div className="flex items-center gap-2 text-slate-800 font-bold">
              <Building className="w-4 h-4 text-slate-500" />
              <span>{nominationInfo.nominatingSociety}</span>
            </div>
            <p className="text-[11px] text-slate-500 tabular-nums">Society Registration No: {nominationInfo.societyRegNo}</p>
          </div>
          ...
          <div className="flex items-center justify-between pt-1">
            ...
            <span className="px-2.5 py-0.5 rounded-full bg-emerald-100 text-emerald-800 font-bold text-[11px] flex items-center gap-1 border border-emerald-300">
              ...
              {nominationInfo.sponsorshipStatus}
            </span>
          </div>
        </div>
        ...
        <div className="space-y-2">
          ...
          <div className="p-3.5 rounded-xl border border-indigo-200 bg-indigo-50/50 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="text-[10px] bg-indigo-50 ring-1 ring-indigo-200 text-indigo-700 px-2 py-0.5 rounded-full tabular-nums">OCT 2026</span>
            </div>
            ...
            <button
              ...
              disabled={registered}
              className="w-full py-2 rounded-full bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors flex items-center justify-center gap-1.5 shadow-2xs disabled:opacity-75"
            >
              {registered ? (
                <>
        ...
        <div className="pt-2 flex items-center justify-between">
          <button
            ...
            className="px-4 py-2 rounded-full bg-white border border-slate-300 hover:bg-indigo-50 hover:border-indigo-300 text-indigo-700 font-bold text-xs flex items-center gap-1.5 transition-colors"
          >
          ...
          <button
            onClick={closeModal}
            className="px-5 py-2 rounded-full bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors"
          >
            Close
          </button>
        </div>
