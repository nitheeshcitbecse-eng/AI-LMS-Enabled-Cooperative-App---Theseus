...
export const AdminProgrammesPage = () => {
  const { programmeOperations } = useData();
  const { openModal } = useApp();
  const [searchTerm, setSearchTerm] = useState('');
  const [statusFilter, setStatusFilter] = useState('ALL');

  const filteredProgrammes = programmeOperations.filter(prog => {
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 ncct-page-header">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-semibold text-slate-900">Programme Operations</h1>
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3.5 py-2.5 rounded-full bg-white border border-slate-300 hover:bg-indigo-50 hover:border-indigo-300 text-indigo-700 font-bold text-xs transition-colors flex items-center gap-1.5"
          >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-2xs space-y-4">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
          ...
          <div className="flex items-center gap-1 bg-slate-100 p-1 rounded-xl text-xs font-bold">
            {(['ALL', 'Active', 'Upcoming', 'Full', 'At Risk'] ).map(status => (
              <button
                key={status}
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredProgrammes.map(item => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-slate-900">{item.filledSeats} / {item.totalSeats}</span>
