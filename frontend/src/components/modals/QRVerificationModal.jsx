...
export const QRVerificationModal = () => {
  ...
  const qrUrl = `https://ncct.gov.in/verify/profile/${trainee.digitalId}`;
      ...
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl ring-1 ring-slate-200 animate-in zoom-in-95 duration-300 space-y-5 relative">
        ...
        <div className="p-4 rounded-xl bg-slate-50 border border-slate-200 text-center space-y-3">
          <div className="w-44 h-44 bg-white border-2 border-indigo-900 rounded-xl p-2 mx-auto flex items-center justify-center shadow-inner relative group">
            <div className="w-full h-full bg-slate-900 p-2 rounded flex flex-col items-center justify-center text-white">
              ...
              <span className="text-[8px] tabular-nums text-slate-300 mt-1">NCCT-BLOCKCHAIN-LEDGER</span>
            </div>
          </div>
        ...
        <div className="space-y-2 text-xs text-slate-700 bg-slate-50/70 p-3 rounded-lg border border-slate-200">
          ...
          <div className="flex justify-between border-b border-slate-200 pb-1">
            ...
            <span className="tabular-nums font-bold text-indigo-900">{trainee.digitalId}</span>
          </div>
          <div className="flex justify-between border-b border-slate-200 pb-1">
          ...
          <div className="flex justify-between">
            ...
            <span className="tabular-nums text-slate-500 text-[10px]">{verificationHash}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 pt-1">
          ...
          <button
            onClick={closeModal}
            className="px-4 py-2.5 rounded-full bg-indigo-600 text-white font-semibold text-xs shadow-sm hover:bg-indigo-700 transition-colors"
          >
            Done
          </button>
        </div>
