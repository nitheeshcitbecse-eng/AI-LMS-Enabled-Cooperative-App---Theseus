  ...
  useEffect(() => {
    if (activeModal === 'face_attendance') {
      ...
      const timer2 = setTimeout(() => {
        ...
        traineeService.checkInAttendance('face').catch(err => console.error('Attendance check-in failed:', err));
      }, 3200);
      return () => {
        clearTimeout(timer1);
        clearTimeout(timer2);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl ring-1 ring-slate-200 animate-in zoom-in-95 duration-300 space-y-5 relative">
        ...
        <div className="text-center space-y-1">
          ...
          <h3 className="text-lg font-semibold text-slate-900">Digital Biometric Attendance Check-In</h3>
        ...
        {activeTab === 'face' ? (
          <div className="relative w-full h-64 rounded-2xl bg-slate-950 overflow-hidden border-2 border-indigo-900 flex flex-col items-center justify-center text-white shadow-inner">
            ...
            <div className={`relative w-40 h-40 rounded-2xl border-2 transition-all duration-500 flex flex-col items-center justify-center ${
              scanStep === 'confirmed'
                ? 'border-emerald-400 bg-emerald-500/20 shadow-lg ring-4 ring-emerald-400/40'
                : scanStep === 'matched'
              ...
              {scanStep === 'confirmed' && (
                <div className="space-y-1 text-center animate-in zoom-in-95 duration-300">
                  ...
                  <span className="text-[10px] font-semibold text-emerald-200 block uppercase tracking-wider">
                    Verified Present!
                  </span>
                </div>
              )}
        ...
        <div className="pt-2">
          <button
            onClick={closeModal}
            className="w-full py-2.5 rounded-full bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors shadow-sm"
          >
