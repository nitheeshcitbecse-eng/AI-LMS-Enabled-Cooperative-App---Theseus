...
export const AnnouncementModal = () => {
  ...
  const handleSend = async () => {
    try {
      await trainerService.sendAnnouncement(announcementText);
    } catch (err) {
      alert(`Could not send announcement: ${err.message}`);
      return;
    }
    setSent(true);
    setTimeout(() => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl ring-1 ring-slate-200 animate-in zoom-in-95 duration-300 space-y-5 relative">
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
          <div className="w-10 h-10 rounded-xl bg-indigo-600 text-amber-400 flex items-center justify-center font-bold shadow-md">
            <Bell className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Send Batch Broadcast Announcement</h3>
        ...
        {!sent ? (
          <div className="space-y-4 text-xs">
            ...
            <div className="pt-2 flex items-center justify-between">
              <button
                onClick={closeModal}
                className="px-4 py-2.5 rounded-full bg-white border border-slate-300 text-indigo-700 font-bold hover:bg-indigo-50 hover:border-indigo-300 transition-colors"
              >
                Cancel
              </button>
              <button
                onClick={handleSend}
                className="px-6 py-2.5 rounded-full bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition-colors flex items-center gap-1.5 shadow-sm"
              >
