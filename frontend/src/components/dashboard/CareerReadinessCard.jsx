...
export const CareerReadinessCard = () => {
  const { careerReadiness } = useData();
  const { openModal } = useApp();

  const breakdownItems = [
    { label: 'Verified Skills', score: careerReadiness.breakdown.verifiedSkills, color: 'bg-emerald-500' },
    { label: 'Learning Progress', score: careerReadiness.breakdown.learningProgress, color: 'bg-teal-500' },
    { label: 'Assessment Score', score: careerReadiness.breakdown.assessmentScore, color: 'bg-indigo-600' },
    { label: 'Credentials', score: careerReadiness.breakdown.credentials, color: 'bg-teal-600' },
    { label: 'Practical Experience', score: careerReadiness.breakdown.experience, color: 'bg-amber-500' },
  ];
  ...
  return (
    <div className="ncct-card p-5 space-y-4">
      ...
      <div className="flex items-center justify-between border-b border-slate-100 pb-3">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Career Readiness Score</h3>
      ...
      <div className="flex items-center gap-5 p-4 rounded-xl bg-slate-50 border border-slate-200">
        <div className="w-20 h-20 rounded-full ncct-chrome text-white flex flex-col items-center justify-center shrink-0 shadow-md">
          <span className="text-2xl font-semibold leading-none">{careerReadiness.overallScore}</span>
          <span className="text-[9px] uppercase tracking-widest text-teal-200 font-bold">Score</span>
        </div>

        <div className="space-y-1">
          <div className="flex items-center gap-1.5 text-xs font-bold text-emerald-800 bg-emerald-100/80 px-2.5 py-0.5 rounded-full inline-block">
            ✓ You're on track for placement
          </div>
          <p className="text-xs text-slate-600 leading-relaxed">
            {careerReadiness.improvementTip}
          </p>
        </div>
      </div>
