...
import { Scan, ShieldCheck, CheckCircle2, XCircle, Clock } from 'lucide-react';

export const TrainerAttendancePage = () => {
  const recentLogs = [
    ...
    <div className="space-y-6 pb-12">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <div className="flex items-center gap-2">
              <h1 className="text-xl font-semibold text-slate-900">Attendance Analytics & Biometric Logs</h1>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="px-3.5 py-2 rounded-full bg-indigo-600 text-white hover:bg-indigo-700 text-xs font-semibold transition-colors flex items-center gap-1.5"
            >
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-2xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Live Biometric & QR Attendance Log</h3>
            <p className="text-xs text-slate-500">Tamper-evident digital attendance stream for Batch CMDO-2026-B04</p>
          </div>
          <span className="text-[10px] tabular-nums font-bold text-teal-700 bg-teal-50 px-2 py-1 rounded-full border border-teal-200">
            ...
            <tbody className="divide-y divide-slate-100">
              {recentLogs.map((log, idx) => (
                <tr key={idx} className="hover:bg-slate-50">
                  <td className="py-3 px-3 text-slate-600 tabular-nums flex items-center gap-1.5">
                  ...
                  <td className="py-3 px-3">
                    {log.status === 'VERIFIED' ? (
                      <span className="px-2 py-0.5 rounded-full bg-emerald-100 text-emerald-800 text-[10px] font-bold flex items-center gap-1 w-max">
                        <CheckCircle2 className="w-3 h-3 text-emerald-600" /> Verified
                      </span>
                    ) : log.status === 'OVERRIDDEN' ? (
                      <span className="px-2 py-0.5 rounded-full bg-amber-100 text-amber-800 text-[10px] font-bold flex items-center gap-1 w-max">
