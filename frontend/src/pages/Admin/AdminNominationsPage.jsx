...
export const AdminNominationsPage = () => {
  const { nominationConflicts } = useData();
    ...
    <div className="space-y-6 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 ncct-page-header">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-semibold text-slate-900">Smart Nomination Control ("One Trainee. One Seat.")</h1>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-1">
          ...
          <div className="text-2xl font-semibold text-slate-900">1,462</div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-2xs space-y-1">
          ...
          <div className="text-2xl font-semibold text-emerald-700">1,418</div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-amber-300 shadow-2xs space-y-1 bg-amber-50/20">
          ...
          <div className="text-2xl font-semibold text-amber-800">44</div>
      ...
      <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-2xs space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <div>
            <h2 className="text-lg font-semibold text-slate-900">Nomination Audit & Conflict Detection Roster</h2>
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs">
            ...
            <tbody className="divide-y divide-slate-100">
              {nominationConflicts.map(item => (
                <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                  <td className="py-3 px-3">
                    <span className="font-bold text-slate-900 block">{item.traineeName}</span>
                    <span className="text-[10px] text-slate-400 tabular-nums">{item.digitalId}</span>
                  </td>
                  ...
                  <td className="py-3 px-3">
                    {item.nominationStatus === 'Confirmed' ? (
                      ...
                      <span className="px-2.5 py-0.5 rounded-full bg-amber-100 text-amber-900 text-[10px] font-semibold border border-amber-300 flex items-center gap-1 w-max animate-pulse">
                  ...
                  <td className="py-3 px-3 text-right">
                    {item.nominationStatus === 'Conflict' ? (
                      <button
                        ...
                        className="px-3 py-1.5 rounded-full bg-indigo-600 text-white font-semibold hover:bg-indigo-700 text-xs shadow-2xs transition-colors"
                      >
                        Resolve Conflict
                      </button>
                    ) : (
