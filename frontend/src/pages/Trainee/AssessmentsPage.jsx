  ...
  return (
    <div className="space-y-6 pb-12">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 ncct-page-header">
        <div>
          <h1 className="text-xl font-semibold text-slate-900">Assessments & Evaluation Ledger</h1>
      ...
      <div className="ncct-card p-6 space-y-4">
        <div className="flex items-center justify-between border-b border-slate-100 pb-3">
          <h3 className="text-lg font-semibold text-slate-900">Completed Assessments ({assessments.length})</h3>
              ...
              >
                ...
                <div className="flex items-center gap-4 shrink-0">
                  <div className="text-right">
                    <span className={`text-lg font-semibold block ${isPassed ? 'text-emerald-700' : 'text-amber-700'}`}>
                      {item.score}%
                    </span>
                  ...
                  {!isPassed && (
                    <button
                      onClick={() => openModal('booster_quiz')}
                      className="px-3 py-1.5 rounded-full bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors shadow-2xs"
                    >
                      Retake Booster
                    </button>
                  )}
