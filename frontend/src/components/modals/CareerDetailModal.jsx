...
export const CareerDetailModal = () => {
  ...
  const handleApply = async () => {
    try {
      await traineeService.applyForOpportunity(selectedOpportunity.id);
    } catch (err) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-xl w-full p-6 shadow-2xl ring-1 ring-slate-200 animate-in zoom-in-95 duration-300 space-y-5 relative max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-2 border-b border-slate-100 pb-4">
          ...
          <h2 className="text-xl font-semibold text-slate-900 leading-tight">{selectedOpportunity.title}</h2>
          ...
          <div className="space-y-1.5">
            {selectedOpportunity.matchedSkills.map(skill => (
              <div key={skill} className="flex items-center justify-between text-xs p-2 rounded-lg bg-emerald-50/70 border border-emerald-200 text-emerald-900 font-semibold">
                ...
                <span className="text-[10px] bg-white px-2 py-0.5 rounded-full border border-emerald-300">
            ...
            {selectedOpportunity.missingSkills.map(skill => (
              <div key={skill} className="flex items-center justify-between text-xs p-2 rounded-lg bg-amber-50/70 border border-amber-200 text-amber-900 font-medium">
                ...
                <span className="text-[10px] bg-white px-2 py-0.5 rounded-full border border-amber-300">
                  Recommended Booster
                </span>
              </div>
            ))}
        ...
        <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
          ...
          {applied ? (
            ...
            <button
              onClick={handleApply}
              className="px-6 py-2.5 rounded-full bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors flex items-center gap-2 shadow-sm"
            >
