      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-slate-100 pb-3">
        <div>
          <div className="flex items-center gap-2">
            <h3 className="text-lg font-semibold text-slate-900">Topic Performance Heatmap Matrix</h3>
            <span className="px-2.5 py-0.5 rounded-full bg-indigo-50 text-indigo-900 text-[10px] font-semibold border border-indigo-200">
              Batch Performance
            </span>
          </div>
        ...
        <button
          ...
          className="px-3.5 py-1.5 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs flex items-center gap-1 transition-colors shadow-2xs shrink-0"
        >
            ...
            {topicHeatmap.map(row => (
              <tr key={row.topicName} className="hover:bg-slate-50 transition-colors">
                ...
                {row.cells.map((cell, idx) => (
                  <td key={idx} className="py-3 px-3 text-center">
                    <span
                      ...
                      className={`inline-block w-14 py-1 rounded font-semibold text-xs transition-transform hover:scale-105 cursor-pointer ${
                        cell.score >= 75
                ...
                <td className="py-3 px-3 text-right">
                  <span className={`font-semibold ${row.averageScore < 60 ? 'text-red-600' : 'text-emerald-700'}`}>
                    {row.averageScore}%
                  </span>
                </td>
              </tr>
