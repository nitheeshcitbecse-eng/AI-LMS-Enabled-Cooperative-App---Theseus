...
export const QuickActionCenter = () => {
  ...
  const actions = [
    ...
    { label: 'Send Announcement', icon: Bell, color: 'bg-indigo-600 text-white', onClick: () => openModal('announcement_modal') },
    { label: 'Upload Resource', icon: BookOpen, color: 'bg-emerald-600 text-white', onClick: () => openModal('trainer_resource_modal') },
    { label: 'Verify Competency', icon: ShieldCheck, color: 'bg-teal-600 text-white', onClick: () => setActiveTab('trainer_competency') },
    { label: 'Batch Reports', icon: BarChart3, color: 'bg-indigo-700 text-white', onClick: () => setActiveTab('trainer_reports') },
  ];

  return (
    <div className="ncct-card p-5 space-y-3">
      <div className="flex items-center justify-between border-b border-slate-100 pb-2">
        <h3 className="text-lg font-semibold text-slate-900">Trainer Quick Action Center</h3>
