  ...
  return (
    <div className="space-y-8 pb-16">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 ncct-page-header">
        <div>
          <div className="flex items-center gap-2">
            <h1 className="text-xl font-semibold text-slate-900">Certificates & Digital Credentials</h1>
        ...
        <button
          onClick={() => handleVerify(certificates[0])}
          className="px-4 py-2.5 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs flex items-center gap-2 shadow-sm transition-colors cursor-pointer"
        >
      ...
      <div className="space-y-4">
        ...
        <DigitallyVerifiedCertificate certificate={certificates[0]} showActions={true} />
      </div>

      {/* Additional Credentials List */}
      <div className="space-y-4 pt-4">
        <h3 className="text-sm font-semibold text-slate-900">All Earned Credentials</h3>
            ...
            <div key={cert.id} className="p-5 bg-white rounded-2xl border border-slate-200 shadow-2xs space-y-3">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="text-sm font-semibold text-slate-900">{cert.title}</h4>
                    <span className="text-[10px] tabular-nums text-slate-500">Credential ID: {cert.credentialId}</span>
                  </div>
                </div>
