...
export const HelpPage = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      <section className="relative overflow-hidden rounded-2xl ncct-chrome px-6 py-7 text-white shadow-lg sm:px-8">
        ...
        <div className="relative max-w-2xl">
          ...
          <button
            ...
            className="mt-5 inline-flex items-center gap-2 rounded-lg bg-white px-4 py-2.5 text-xs font-semibold text-indigo-700 shadow-sm transition hover:bg-indigo-50"
          >
      ...
      <section className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        {[
          { icon: BookOpen, title: 'Learning guides', text: 'Browse course and assessment guidance.', color: 'text-indigo-600 bg-indigo-50' },
          ...
          <div key={item.title} className="ncct-card flex items-start gap-3 p-4">
            ...
            <div><h3 className="text-lg font-semibold text-slate-900">{item.title}</h3><p className="mt-1 text-xs leading-relaxed text-slate-500">{item.text}</p></div>
          </div>
        ))}
      </section>
      ...
      <section className="ncct-card p-5 sm:p-6">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          {faqs.map((faq, i) => (
            <article key={i} className="rounded-xl border border-slate-200 bg-white p-4 transition hover:border-indigo-200 hover:bg-indigo-50/40">
              <h3 className="flex items-start gap-2 text-sm font-bold leading-snug text-[#0c4a6e]"><HelpCircle className="mt-0.5 h-4 w-4 shrink-0 text-teal-600" /><span>{faq.q}</span></h3>
