...
export const AIAssistantDrawer = () => {
  ...
  const getGreetingText = () => {
    if (language === 'ta') return `வணக்கம் ${trainee.name.split(' ')[0]} 👋 நான் உங்கள் NCCT AI கற்றல் வழிகாட்டி. உங்கள் மதிப்பீடுகள் மற்றும் திறன் நிலையை நான் கண்காணிக்கிறேன். இன்று உங்களுக்கு எவ்வாறு உதவட்டும்?`;
    if (language === 'hi') return `नमस्ते ${trainee.name.split(' ')[0]} 👋 मैं आपका NCCT AI शिक्षण मार्गदर्शक हूँ। मैं आपके मूल्यांकन और कौशल अंतर की निगरानी करता हूँ। आज मैं आपकी क्या सहायता कर सकता हूँ?`;
    if (language === 'te') return `నమస్తే ${trainee.name.split(' ')[0]} 👋 నేను మీ NCCT AI అభ్యాస మార్గదర్శకుడిని. నేను మీ మూల్యాంకనాలు మరియు నైపుణ్య లోపాలను పర్యవేక్షిస్తాను. ఈ రోజు నేను మీకు ఎలా సహాయపడగలను?`;
    if (language === 'bn') return `নমস্কার ${trainee.name.split(' ')[0]} 👋 আমি আপনার NCCT এআই শিক্ষা গাইড। আমি আপনার মূল্যায়ন এবং দক্ষতার ব্যবধান পর্যবেক্ষণ করি। আজ আমি আপনাকে কীভাবে সাহায্য করতে পারি?`;
    return `Hi ${trainee.name.split(' ')[0]} 👋 I am your contextual NCCT AI Learning Guide. I monitor your assessment patterns, skill gaps, and career roadmap. How can I assist you today?`;
  };
  ...
  const handleSend = (textToSend) => {
    ...
    if (aiService.isEnabled) {
      aiService
        .sendMessage({ role: 'trainee', message: query, language })
    ...
    <>
      ...
      <div className="p-4 ncct-chrome text-white flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <h3 className="text-[13px] font-medium text-white leading-tight">NCCT AI Learning Assistant</h3>
            <p className="text-[11px] text-slate-400">Contextual Trainee Guide</p>
          </div>
        </div>
      ...
      <div className="p-3 border-t border-slate-200 bg-white space-y-2">
        <span className="text-[10px] uppercase tracking-[0.08em] font-medium text-slate-400 block">Suggested Contextual Queries</span>
        <div className="flex flex-wrap gap-1.5">
          {suggestedPrompts.map(prompt => (
            <button
              key={prompt}
              onClick={() => handleSend(prompt)}
              className="text-[11px] px-2.5 py-1.5 rounded-lg bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-900 ring-1 ring-slate-200 hover:ring-slate-300 transition-all duration-200 text-left"
            >
              {prompt}
            </button>
          ))}
      ...
      <div className="p-3 bg-white border-t border-slate-200 flex items-center gap-2">
        <input
          ...
          className="flex-1 px-3 py-2 text-xs bg-slate-100/70 border border-transparent rounded-lg placeholder:text-slate-400 transition-all duration-200 focus:outline-none focus:bg-white focus:border-slate-300 focus:ring-4 focus:ring-indigo-500/10"
        />
        <button
          onClick={() => handleSend()}
          className="ncct-press p-2.5 rounded-full bg-indigo-600 text-white hover:bg-indigo-700 shrink-0"
        >
