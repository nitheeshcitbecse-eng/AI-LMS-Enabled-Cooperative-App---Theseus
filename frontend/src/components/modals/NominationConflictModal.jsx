...
export const NominationConflictModal = () => {
  ...
  const handleResolve = async (choice) => {
    try {
      await adminService.resolveNominationConflict(trainee.id || trainee.digitalId, choice);
    } catch (err) {
      alert(`Could not resolve conflict: ${err.message}`);
      return;
    }
  ...
  return (
    <div className="fixed inset-0 z-50 bg-slate-950/60 backdrop-blur-md flex items-center justify-center p-4 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-lg w-full ring-1 ring-slate-200 shadow-2xl animate-in zoom-in-95 duration-300 overflow-hidden space-y-4">
        {/* Header */}
        <div className="p-4 bg-amber-500 text-slate-950 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-sm font-semibold">Nomination Conflict Detected</h3>
          </div>
        ...
        <div className="p-5 space-y-4 text-xs">
          <div className="bg-slate-50 p-3 rounded-xl border border-slate-200 space-y-1">
            ...
            <div className="flex items-center justify-between">
              <span className="font-semibold text-slate-900 text-sm">{trainee.traineeName}</span>
              <span className="tabular-nums text-indigo-900 bg-indigo-50 px-2 py-0.5 rounded border border-indigo-100 font-bold">
                {trainee.digitalId}
              </span>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="p-3.5 rounded-xl bg-slate-50 border border-slate-200 space-y-1">
              <span className="text-[10px] text-slate-400 font-semibold uppercase block">Existing Nomination</span>
              ...
              <p className="text-[10px] text-slate-500 tabular-nums">{trainee.existingProgramme?.dates}</p>
            </div>

            {/* New Nomination Request */}
            <div className="p-3.5 rounded-xl bg-amber-50 border border-amber-300 space-y-1">
              <span className="text-[10px] text-amber-800 font-semibold uppercase block">New Request (ICM Chennai)</span>
              ...
              <p className="text-[10px] text-slate-500 tabular-nums">{trainee.newProgramme?.dates}</p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-2">
            <button
              ...
              className="py-2 px-3 rounded-full bg-white border border-slate-300 hover:bg-indigo-50 hover:border-indigo-300 text-indigo-700 font-bold text-xs transition-colors"
            >
              Keep Existing
            </button>
            ...
            <button
              ...
              className="py-2 px-3 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs transition-colors shadow-2xs"
            >
              Accept New (Chennai)
            </button>
            ...
            <button
              ...
              className="py-2 px-3 rounded-full bg-indigo-600 hover:bg-indigo-700 text-white font-semibold text-xs transition-colors"
            >
              Contact Coordinator
            </button>
          </div>
