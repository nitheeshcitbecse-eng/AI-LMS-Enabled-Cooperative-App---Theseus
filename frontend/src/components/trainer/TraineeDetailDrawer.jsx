...
export const TraineeDetailDrawer = () => {
  const {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-full sm:w-96 md:w-[480px] bg-white shadow-2xl border-l border-slate-200 flex flex-col animate-in slide-in-from-right duration-300">
      {/* Drawer Header */}
      <div className="p-5 ncct-chrome text-white flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs tabular-nums text-indigo-200">NCCT ID: {trainee.digitalId}</p>
          </div>
        </div>

        <button
          onClick={() => setSelectedTraineeForDrawer(null)}
          className="p-1.5 rounded-lg text-indigo-300 hover:text-white hover:bg-white/10"
        >
          <X className="w-5 h-5" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5 bg-slate-50/50">
        ...
        <div className="grid grid-cols-3 gap-2 text-center p-3 rounded-xl bg-white border border-slate-200 shadow-2xs">
          <div>
            ...
            <span className="text-base font-semibold text-emerald-700">{trainee.attendance}%</span>
          </div>
          <div className="border-x border-slate-200">
            <span className="text-[10px] text-slate-500 font-bold uppercase block">Assessment Avg</span>
            <span className={`text-base font-semibold ${displayScore < 60 ? 'text-red-600' : 'text-emerald-700'}`}>
              {displayScore}%
            </span>
          </div>
          <div>
            <span className="text-[10px] text-slate-500 font-bold uppercase block">LMS Activity</span>
            <span className="text-base font-semibold text-indigo-900">{trainee.lmsActivity}%</span>
          </div>
        </div>
        ...
        <div className="ncct-card p-4 space-y-3">
          ...
          <div className="space-y-2">
            {currentNotes.map(n => (
              <div key={n.id} className="p-2.5 rounded-lg bg-slate-50 border border-slate-200 text-xs text-slate-700 space-y-1">
                <p>{n.noteText}</p>
                <span className="text-[10px] text-slate-400 block tabular-nums">{n.createdAt}</span>
              </div>
            ))}
          </div>
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              className="px-3 py-1.5 rounded-full bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors"
            >
              Save
            </button>
          </div>
      ...
      <div className="p-4 bg-white border-t border-slate-200 flex items-center justify-between gap-2">
        <button
          ...
          className="flex-1 py-2.5 rounded-full bg-indigo-600 text-white font-semibold text-xs hover:bg-indigo-700 transition-colors flex items-center justify-center gap-1.5 shadow-sm"
        >
        ...
        <button
          ...
          className="p-2.5 rounded-full bg-white border border-slate-300 hover:bg-indigo-50 hover:border-indigo-300 text-indigo-700 transition-colors"
