...
export const TrainerResourceModal = () => {
  ...
  const handleUpload = async () => {
    try {
      await trainerService.uploadResource({ title: resourceTitle });
    } catch (err) {
      alert(`Could not upload resource: ${err.message}`);
      return;
    }
    setUploaded(true);
    setTimeout(() => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-md animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-md w-full p-6 shadow-2xl ring-1 ring-slate-200 animate-in zoom-in-95 duration-300 space-y-5 relative">
        ...
        <div className="flex items-center gap-3 border-b border-slate-100 pb-3">
          ...
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Upload & Assign Teaching Resource</h3>
          ...
          <div className="space-y-4 text-xs">
            ...
            <div className="pt-2 flex items-center justify-between">
              ...
              <button
                ...
                className="px-6 py-2.5 rounded-full bg-indigo-600 text-white font-semibold hover:bg-indigo-700 transition-colors shadow-sm"
              >
